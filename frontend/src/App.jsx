...
const useVideoCall = (socket, activeChatRoom, user) => {
  ...
  useEffect(() => {
    ...
    const handleIncoming = (data) =>
      setCallState({ status: 'receiving', offer: data.offer, callerName: data.callerName, room: data.room });
    ...
    <div className="bg-gray-50 w-full h-screen relative overflow-hidden flex flex-col justify-center items-center px-6">
      <div className="w-full max-w-xl mx-auto">
        <div className="text-center mb-8">
          <div
            className={`w-16 h-16 ${
              isProMode ? 'bg-gray-800' : 'bg-teal-600'
          ...
          <div className="flex bg-gray-100 p-1 rounded-xl mb-6">
            <button
              ...
              className={`flex-1 py-2 text-sm font-bold rounded-lg transition ${
                !isProMode ? 'bg-white shadow-sm text-teal-600' : 'text-gray-500'
              }`}
            >
              Client
            </button>
            <button
              ...
              className={`flex-1 py-2 text-sm font-bold rounded-lg transition ${
                isProMode ? 'bg-gray-800 shadow-sm text-white' : 'text-gray-500'
              }`}
            >
              Professional
            </button>
        ...
        <form
          ...
          className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 max-h-[60vh] overflow-y-auto hide-scrollbar w-full"
        >
          ...
          {!isLogin && (
            <div className="mb-3">
              ...
              <input
                ...
                className="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl mt-1 outline-none text-sm"
              />
            </div>
          )}
          ...
          <button
            ...
            disabled={isLoading}
            className={`w-full ${
              isProMode && !isLogin ? 'bg-gray-800' : 'bg-teal-600'
            } text-white font-bold py-3.5 rounded-xl transition shadow-md mt-2`}
          >
...
const ClientApp = ({ socket, token }) => {
  ...
  const markNotificationsRead = () => {
    ...
    if (!showNotifications && unreadCount > 0) {
      fetch(`${backendUrl}/api/notifications/read`, {
        method: 'PATCH',
        headers: { Authorization: `Bearer ${token}` }
      });
      setNotifications(
  ...
  const handleCancelBooking = async (id) => {
    ...
    const res = await fetch(`${backendUrl}/api/bookings/${id}/cancel`, {
      method: 'PATCH',
      headers: { Authorization: `Bearer ${token}` }
    });
    if (res.ok) {
  ...
  const openPrivateChat = async (booking) => {
    ...
    setMessageList([]);
    setActiveTab('chat');
    fetch(`${backendUrl}/api/chat/${booking._id}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
      .then(res => res.json())
  ...
  const categories = [
    { id: 'cleaning', name: 'Cleaning', icon: 'fa-broom', bg: 'bg-blue-50', color: 'text-blue-500' },
    { id: 'electric', name: 'Electric', icon: 'fa-bolt', bg: 'bg-orange-50', color: 'text-orange-500' },
    { id: 'plumbing', name: 'Plumbing', icon: 'fa-wrench', bg: 'bg-teal-50', color: 'text-teal-600' },
    { id: 'tech', name: 'Tech & IT', icon: 'fa-laptop-code', bg: 'bg-purple-50', color: 'text-purple-500' }
  ];

  let displayedPros = [];
  if (activeTab === 'favorites') {
    displayedPros = professionals.filter(p => favorites.includes(p._id));
  } else {
    displayedPros = selectedCategory
      ? professionals.filter(p => p.category === selectedCategory)
      : professionals;
  }

  const filteredPros = displayedPros.filter(p =>
  ...
  const handleAvatarUpload = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        login({ ...user, avatar: data.avatar }, token);
      }
    } catch (err) {}
  };
    ...
    <div className="bg-gray-50 w-full h-screen relative flex flex-col text-gray-900 overflow-hidden font-sans">
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <button
              ...
              className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-gray-600 relative"
            >
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map(n => (
                      <div
                        ...
                        className={`px-6 py-3 border-b border-gray-50 cursor-pointer hover:bg-gray-100 transition ${
          ...
          <img
            src={
              user.avatar ||
              `https://ui-avatars.com/api/?name=${user.name.replace(/ /g, '+')}&background=0D8ABC&color=fff`
            }
            className="w-10 h-10 rounded-full shadow-sm cursor-pointer object-cover"
      ...
      <div className="flex-1 w-full h-full flex flex-col overflow-hidden relative">
        {activeTab === 'home' && (
          <div className="flex-1 overflow-y-auto px-6 pt-6 pb-28 flex flex-col w-full hide-scrollbar">
            <div className="relative mb-6 shadow-sm w-full">
              ...
              <input
                ...
                className="w-full bg-white py-3.5 pl-12 pr-4 rounded-2xl text-sm outline-none border border-gray-100"
              />
            </div>
            ...
            <div className="flex flex-col gap-4 w-full">
              {paginatedPros.map(pro => (
                <div
                  ...
                  className="w-full bg-white p-4 rounded-2xl flex items-center shadow-sm border border-gray-50 cursor-pointer hover:shadow-md transition"
                >
                  <div className="relative flex-shrink-0">
                    ...
                    />
                    {pro.verified && (
                      <div className="absolute -top-2 -right-2 bg-blue-500 text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px] border-2 border-white">
                  ...
                  <div className="ml-4 flex-1">
                    <div>
                      <h3 className="font-bold text-gray-800 text-base">{pro.name}</h3>
                      <p className="text-xs text-teal-600 font-bold">{pro.title}</p>
                    </div>
                    <div className="flex items-center mt-2 text-xs text-gray-500 font-medium">
                      <span className="flex items-center text-orange-500 mr-3">
                        <i className="fas fa-star mr-1"></i> {pro.rating}
                      </span>
                      <span className="flex items-center">
                        <i className="fas fa-map-marker-alt mr-1"></i> {pro.distance}
                      </span>
                    </div>
                  </div>
            ...
            {paginatedBookings.map(b => (
              ...
              >
                ...
                <div className="flex gap-2 mt-3 w-full">
                  <button
                    onClick={() => openPrivateChat(b)}
                    className="flex-1 py-2 bg-teal-50 text-teal-600 text-xs font-bold rounded-lg hover:bg-teal-100 transition"
                  >
                    <i className="fas fa-comment-dots mr-1"></i> Chat
                  </button>
                  {b.status === 'pending' && (
                    <button
                      onClick={() => handleCancelBooking(b._id)}
                      className="flex-1 py-2 bg-red-50 text-red-600 text-xs font-bold rounded-lg hover:bg-red-100 transition"
                    >
                      Cancel
                    </button>
                  )}
          ...
          <div className="flex-1 flex flex-col w-full h-full bg-gray-50">
            <div className="bg-white px-6 py-4 flex justify-between items-center shadow-sm shrink-0 w-full">
              <div className="flex items-center">
                <button
                  ...
                  className="mr-4 text-gray-400 hover:text-teal-600 transition"
                >
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="w-8 h-8 bg-teal-50 text-teal-600 rounded-full flex items-center justify-center text-xs hover:bg-teal-100 transition"
                >
                  <i className="fas fa-video"></i>
                </button>
              </div>
            ...
            <div className="bg-white py-4 px-6 border-t border-gray-100 flex items-center gap-3 shrink-0 w-full">
              <div className="relative shrink-0">
                <button
                  onClick={() => setViewingLiveMap(true)}
                  className="w-10 h-10 bg-gray-50 rounded-full text-gray-400 flex items-center justify-center hover:bg-gray-100 transition"
                >
              ...
              <input
                ...
                className="flex-1 w-full bg-gray-50 py-3 px-4 rounded-full text-sm outline-none border border-gray-100 focus:ring-1 focus:ring-teal-500"
              />
              <button
                onClick={sendMessage}
        ...
        {activeTab === 'chat' && !activeChatRoom && (
          <div className="flex-1 overflow-y-auto px-6 flex flex-col items-center justify-center pb-28 w-full">
            <div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mb-4">
              <i className="fas fa-comments text-3xl opacity-50"></i>
            </div>
            ...
            <button
              onClick={() => setActiveTab('bookings')}
              className="mt-6 px-6 py-2 bg-teal-50 text-teal-600 rounded-xl font-bold text-xs hover:bg-teal-100 transition"
            >
              Go to Bookings
            </button>
          </div>
        ...
        {activeTab === 'profile' && (
          <div className="flex-1 overflow-y-auto px-6 pt-6 pb-28 flex flex-col w-full hide-scrollbar">
            ...
            <div className="w-full bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center mb-6">
              ...
              <div className="relative mb-4">
                ...
                <button
                  ...
                  className="absolute bottom-0 right-0 w-8 h-8 bg-teal-500 text-white rounded-full flex items-center justify-center border-2 border-white shadow-sm hover:bg-teal-600 transition"
                >
            ...
            <button
              onClick={logout}
              className="w-full py-4 bg-red-50 text-red-500 font-bold rounded-2xl border border-red-100 flex items-center justify-center hover:bg-red-100 transition"
            >
        ...
        <div className="absolute inset-0 w-full h-full bg-white z-[60] overflow-y-auto flex flex-col animate-[slideUp_0.3s_ease-out] hide-scrollbar">
          <div className="w-full relative h-64 bg-gray-100 px-6 shrink-0">
            ...
            <button
              ...
              className="absolute top-6 left-6 w-10 h-10 bg-white/20 backdrop-blur-md rounded-full flex items-center justify-center text-white border border-white/30 hover:bg-white/40"
            >
              <i className="fas fa-arrow-left"></i>
            </button>
          </div>
          <div className="w-full px-6 -mt-16 relative z-10 pb-10">
            <div className="w-full bg-white rounded-3xl p-6 shadow-xl border border-gray-50">
              ...
              <button
                ...
                className="w-full bg-teal-600 text-white font-bold py-4 rounded-2xl shadow-lg shadow-teal-500/30 active:scale-95 hover:bg-teal-700 transition"
              >
                Book Now
              </button>
            </div>
            ...
            <form onSubmit={handleBookingSubmit} className="w-full">
              <div className="mb-4 w-full">
                ...
                <input
                  ...
                  className="w-full bg-white border border-gray-200 p-4 rounded-xl mt-1 outline-none text-sm font-medium shadow-sm"
                />
              </div>
              <div className="mb-4 w-full">
                <label className="text-xs font-bold text-gray-500 ml-1">Select Time</label>
                <input
                  type="time"
                  required
              ...
              <div className="mb-8 w-full">
                <label className="text-xs font-bold text-gray-500 ml-1">Service Address</label>
                <textarea
                  required
                  placeholder="Enter full address..."
                  value={bookingData.address}
                  onChange={e => setBookingData({ ...bookingData, address: e.target.value })}
                  className="w-full bg-white border border-gray-200 p-4 rounded-xl mt-1 outline-none text-sm font-medium shadow-sm h-24 resize-none"
                />
              </div>
              <button
                type="submit"
                className="w-full bg-teal-600 text-white font-bold py-4 rounded-2xl shadow-lg shadow-teal-500/30 hover:bg-teal-700 transition"
              >
                Confirm Booking
              </button>
            </form>
      ...
      {isBookingSuccess && (
        <div className="absolute inset-0 w-full h-full bg-teal-600 z-[80] flex flex-col items-center justify-center px-6 py-8 text-center animate-[fadeIn_0.3s_ease-out]">
          <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center text-teal-600 text-4xl mb-6 shadow-2xl animate-[bounce_1s_ease-out]">
            <i className="fas fa-check"></i>
          </div>
          ...
          <button
            ...
            className="w-full bg-white text-teal-600 font-black py-4 px-12 rounded-2xl shadow-xl hover:bg-gray-50 transition"
          >
            View My Bookings
          </button>
        </div>
        ...
        <div className="absolute inset-0 w-full h-full bg-white z-[90] flex flex-col animate-[slideUp_0.3s_ease-out]">
          ...
          <div className="flex-1 w-full relative min-h-0">
            ...
            <button
              ...
              className={`absolute bottom-6 flex items-center gap-2 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-full text-white font-bold text-sm shadow-xl z-[400] transition ${
                isSharingLocation ? 'bg-red-500 hover:bg-red-600' : 'bg-teal-600 hover:bg-teal-700'
              }`}
            >
...
const ProfessionalApp = ({ socket, token }) => {
  ...
  useEffect(() => {
    ...
    if (token) {
      fetch(`${backendUrl}/api/pro/bookings`, {
        headers: { Authorization: `Bearer ${token}` }
      })
        .then(res => res.json())
  ...
  const openChat = async (job) => {
    ...
    setMessageList([]);
    setActiveTab('chat');
    fetch(`${backendUrl}/api/chat/${job._id}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
      .then(res => res.json())
      .then(data => setMessageList(Array.isArray(data) ? data : []));
  ...
  const updateJobStatus = async (id, status) => {
    const res = await fetch(`${backendUrl}/api/admin/bookings/${id}/status`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`
          ...
          <div className="flex-1 flex flex-col w-full h-full bg-gray-900">
            <div className="bg-gray-800 px-6 py-4 flex justify-between items-center shadow-sm shrink-0 w-full">
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="font-bold text-white text-sm">{activeChatRoom.clientName}</h3>
                  <p className="text-[10px] text-teal-400 font-bold">Job Chat</p>
                </div>
              </div>
              <div className="flex gap-3">
                <button
        ...
        {activeTab === 'chat' && !activeChatRoom && (
          <div className="flex-1 overflow-y-auto px-6 flex flex-col items-center justify-center pb-28 w-full">
            <div className="w-20 h-20 bg-gray-800 rounded-full flex items-center justify-center mb-4">
              <i className="fas fa-comments text-3xl opacity-50 text-teal-500"></i>
            </div>
...
const AppContent = () => {
  ...
  return user.role === 'professional' ? (
    <ProfessionalApp socket={socket} token={token} />
  ) : (
    <ClientApp socket={socket} token={token} />
  );
};
