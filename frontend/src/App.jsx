...
const ClientApp = () => {
  ...
  return (
    <div className="bg-bgLight w-full max-w-md mx-auto h-screen md:h-[850px] md:rounded-[2.5rem] md:shadow-2xl relative overflow-hidden md:border-8 md:border-gray-900 flex flex-col">
        ...
        {/* CLIENT LIVE MAP OVERLAY + SHARE BUTTON */}
        {viewingLiveMap && partnerLocation && (
            <div className="absolute inset-0 bg-white z-50 flex flex-col">
                <div className="p-6 border-b flex justify-between items-center">
                    <button onClick={() => setViewingLiveMap(false)} className="h-10 w-10 rounded-full bg-gray-100"><i className="fas fa-arrow-left"></i></button>
                    <h2 className="text-lg font-bold text-gray-800">Live Map</h2>
                    <div className="w-10"></div>
                </div>
                <div className="flex-1 relative">
                    <MapContainer center={[partnerLocation.lat, partnerLocation.lng]} zoom={16} style={{ height: '100%', width: '100%', zIndex: 1 }}><TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" /><Marker position={[partnerLocation.lat, partnerLocation.lng]}><Popup>{partnerLocation.author}</Popup></Marker></MapContainer>
                    {/* NEW: Map Overlay Share Button */}
                    <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-[400] w-[90%]">
                        <button onClick={toggleLocationSharing} className={`w-full py-4 rounded-2xl font-bold shadow-lg flex items-center justify-center gap-2 ${isSharingLocation ? 'bg-red-500 text-white' : 'bg-teal-600 text-white'}`}>
                            <i className="fas fa-location-arrow"></i> {isSharingLocation ? 'Stop Sharing' : 'Share My Location'}
                        </button>
                    </div>
                </div>
            </div>
        ...
        {viewingProfile && !bookingPro && (
             <div className="absolute inset-0 bg-gray-50 z-40 flex flex-col overflow-y-auto hide-scrollbar">
                 ...
                 <div className="relative bg-white pb-6 shadow-sm border-b"><div className="h-32 w-full bg-gray-200">{viewingProfile.banner && <img src={viewingProfile.banner} className="w-full h-full object-cover" />}</div><div className="px-6 relative"><img src={viewingProfile.avatar} className="w-24 h-24 rounded-full border-4 border-white absolute -top-12 shadow-md object-cover" /><div className="pt-14"><h1 className="text-2xl font-bold text-gray-900">{viewingProfile.name} {viewingProfile.verified && <i className="fas fa-check-circle text-teal-600 text-sm ml-1"></i>}</h1><p className="text-sm font-medium text-gray-800 mt-1">{viewingProfile.headline || viewingProfile.title}</p><p className="text-xs text-gray-500 mt-1"><i className="fas fa-map-marker-alt mr-1"></i> Kano, NG • <span className="font-bold text-teal-600">{viewingProfile.distance}</span></p><div className="flex items-center mt-3 gap-2"><span className="bg-green-50 text-green-700 font-bold px-3 py-1 rounded-lg text-xs">₦{viewingProfile.price.toLocaleString()}/hr</span><span className="bg-orange-50 text-orange-600 font-bold px-3 py-1 rounded-lg text-xs"><i className="fas fa-star mr-1"></i>{viewingProfile.rating}</span></div><button onClick={() => setBookingPro(viewingProfile)} className="w-full bg-teal-600 text-white font-bold py-3.5 rounded-xl mt-5 shadow-md shadow-teal-600/20">Book Now</button></div></div></div>
                 <div className="p-4 flex flex-col gap-4">
                     ...
                     {viewingProfile.contactInfo && Object.values(viewingProfile.contactInfo).some(v => v) && (<div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-20"><h2 className="text-sm font-bold text-gray-900 mb-3">Contact Info</h2><div className="grid grid-cols-2 gap-3">{viewingProfile.contactInfo.portfolio && <a href={viewingProfile.contactInfo.portfolio} target="_blank" rel="noreferrer" className="flex items-center text-xs text-gray-600 bg-gray-50 p-2 rounded-lg"><i className="fas fa-globe text-gray-400 w-5"></i> Portfolio</a>}{viewingProfile.contactInfo.github && <a href={viewingProfile.contactInfo.github} target="_blank" rel="noreferrer" className="flex items-center text-xs text-gray-600 bg-gray-50 p-2 rounded-lg"><i className="fab fa-github text-gray-400 w-5"></i> GitHub</a>}{viewingProfile.contactInfo.linkedin && <a href={viewingProfile.contactInfo.linkedin} target="_blank" rel="noreferrer" className="flex items-center text-xs text-gray-600 bg-gray-50 p-2 rounded-lg"><i className="fab fa-linkedin text-blue-600 w-5"></i> LinkedIn</a>}</div></div>)}
                 </div>
             </div>
        )}
        {bookingPro && (<div className="absolute inset-0 bg-white z-50 flex flex-col"><div className="flex justify-between items-center p-6 border-b"><h2 className="font-bold text-xl">Book {bookingPro.name.split(' ')[0]}</h2><button onClick={() => { setBookingPro(null); setIsBookingSuccess(false); }} className="bg-gray-100 h-10 w-10 rounded-full"><i className="fas fa-times"></i></button></div>{isBookingSuccess ? (<div className="flex-1 flex flex-col items-center justify-center p-6"><div className="w-20 h-20 bg-green-100 text-green-500 rounded-full flex items-center justify-center text-4xl mb-4"><i className="fas fa-check"></i></div><h2 className="text-2xl font-bold mb-2">Booking Confirmed!</h2><p className="text-gray-500 mb-8 text-center">Your request has been sent.</p><button onClick={() => { setBookingPro(null); setIsBookingSuccess(false); setActiveTab('bookings'); }} className="w-full bg-teal-600 text-white font-bold py-4 rounded-2xl shadow-lg">View Bookings</button></div>) : (<form onSubmit={handleBookingSubmit} className="flex-1 p-6 flex flex-col overflow-y-auto"><label className="text-xs font-bold text-gray-500 mb-1 ml-1">Select Date</label><input type="date" required className="w-full bg-gray-50 p-4 rounded-xl mb-6 outline-none" value={bookingData.date} onChange={e => setBookingData({...bookingData, date: e.target.value})} /><label className="text-xs font-bold text-gray-500 mb-1 ml-1">Select Time</label><div className="grid grid-cols-3 gap-3 mb-6">{['10:00 AM', '1:00 PM', '4:00 PM'].map(time => <div key={time} onClick={() => setBookingData({...bookingData, time})} className={`text-center py-3 rounded-xl text-sm font-medium cursor-pointer transition ${bookingData.time === time ? 'bg-teal-600 text-white shadow-md' : 'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}>{time}</div>)}</div><label className="text-xs font-bold text-gray-500 mb-1 ml-1">Service Address</label><textarea required className="w-full bg-gray-50 p-4 rounded-xl mb-6 h-28 outline-none" value={bookingData.address} onChange={e => setBookingData({...bookingData, address: e.target.value})} placeholder="E.g. Zoo Road, Kano"></textarea><div className="bg-gray-50 p-4 rounded-xl mb-6 flex justify-between items-center"><span className="text-sm font-bold text-gray-600">Total Price</span><span className="text-lg font-bold text-teal-600">₦{bookingPro.price.toLocaleString()}</span></div><button type="submit" className="w-full bg-teal-600 text-white font-bold py-4 rounded-2xl mt-auto shadow-lg shadow-teal-600/30">Confirm Booking</button></form>)}</div>)}
...
const ProfessionalApp = () => {
    ...
    // Adjusted Map Viewer to automatically link to the job's chat room for signaling
    const handleViewMap = async (job) => { 
        setActiveChatRoom(job); // Pre-load chat room so location socket knows where to broadcast
        setViewingMapForJob(job); 
        try { const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(job.address)}`); const data = await res.json(); if (data.length > 0) setMapPosition([parseFloat(data[0].lat), parseFloat(data[0].lon)]); } catch (err) {} 
    };

    const toggleLocationSharing = () => { 
        if (!activeChatRoom) return alert("Open the chat room first to establish connection.");
        if (isSharingLocation) { 
            ...
            if (navigator.geolocation) { 
                watchIdRef.current = navigator.geolocation.watchPosition((pos) => { socket.emit('live_location_update', { room: activeChatRoom._id, lat: pos.coords.latitude, lng: pos.coords.longitude, author: user.name }); }, () => {}, { enableHighAccuracy: true }); 
    ...
    return (
        <div className="bg-gray-900 w-full max-w-md mx-auto h-screen md:h-[850px] relative flex flex-col text-white md:rounded-[2.5rem] md:shadow-2xl overflow-hidden">
            ...
            {activeTab === 'profile' && isEditingProfile && (<form onSubmit={handleSaveProfile} className="flex-1 overflow-y-auto px-6 py-6 pb-28 bg-gray-900 relative z-30"><div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold">Edit Portfolio</h2><button type="button" onClick={() => { setIsEditingProfile(false); setEditForm(myProfile); }} className="text-gray-400"><i className="fas fa-times"></i></button></div><div className="space-y-5"><div><label className="text-xs font-bold text-gray-400 mb-1 block">Headline (Your Value Proposition)</label><input type="text" value={editForm.headline || ''} onChange={e => setEditForm({...editForm, headline: e.target.value})} className="w-full bg-gray-800 border border-gray-700 p-3 rounded-xl text-sm text-white outline-none focus:border-teal-500" /></div><div><label className="text-xs font-bold text-gray-400 mb-1 block">About Me (Elevator Pitch)</label><textarea value={editForm.about || ''} onChange={e => setEditForm({...editForm, about: e.target.value})} className="w-full bg-gray-800 border border-gray-700 p-3 rounded-xl text-sm text-white h-24 outline-none focus:border-teal-500"></textarea></div><div><label className="text-xs font-bold text-gray-400 mb-1 block">Skills (Comma separated)</label><input type="text" value={editForm.skills ? editForm.skills.join(', ') : ''} onChange={e => setEditForm({...editForm, skills: e.target.value.split(',').map(s=>s.trim()).filter(s=>s)})} className="w-full bg-gray-800 border border-gray-700 p-3 rounded-xl text-sm text-white outline-none focus:border-teal-500" /></div><div><label className="text-xs font-bold text-gray-400 mb-1 block">GitHub Link</label><input type="text" value={editForm.contactInfo?.github || ''} onChange={e => setEditForm({...editForm, contactInfo: {...(editForm.contactInfo||{}), github: e.target.value}})} className="w-full bg-gray-800 border border-gray-700 p-3 rounded-xl text-sm text-white outline-none focus:border-teal-500" /></div><div><label className="text-xs font-bold text-gray-400 mb-1 block">LinkedIn Link</label><input type="text" value={editForm.contactInfo?.linkedin || ''} onChange={e => setEditForm({...editForm, contactInfo: {...(editForm.contactInfo||{}), linkedin: e.target.value}})} className="w-full bg-gray-800 border border-gray-700 p-3 rounded-xl text-sm text-white outline-none focus:border-teal-500" /></div></div><button type="submit" disabled={isSaving} className="w-full bg-teal-600 text-white font-bold py-4 rounded-2xl mt-8 shadow-lg shadow-teal-600/20">{isSaving ? 'Saving...' : 'Save Portfolio'}</button></form>)}
            ...
            {/* PRO LIVE MAP OVERLAY + SHARE BUTTON */}
            {viewingLiveMap && partnerLocation && (
                <div className="absolute inset-0 bg-gray-900 z-50 flex flex-col">
                    <div className="p-6 border-b border-gray-800 flex justify-between items-center">
                        <button onClick={() => setViewingLiveMap(false)} className="h-10 w-10 rounded-full bg-gray-800 text-gray-400"><i className="fas fa-arrow-left"></i></button>
                    ...
                    <div className="flex-1 relative w-full bg-gray-800">
                        ...
                        {/* NEW: Map Overlay Share Button */}
                        <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-[400] w-[90%]">
                            <button onClick={toggleLocationSharing} className={`w-full py-4 rounded-2xl font-bold shadow-lg flex items-center justify-center gap-2 ${isSharingLocation ? 'bg-red-500/90 text-white border border-red-400' : 'bg-teal-600 text-white'}`}>
                                <i className="fas fa-location-arrow"></i> {isSharingLocation ? 'Stop Sharing' : 'Share My Location'}
                            </button>
                        </div>
                    </div>
            ...
            {/* PRO GEOCODING MAP OVERLAY + SHARE BUTTON */}
            {viewingMapForJob && (
                <div className="absolute inset-0 bg-gray-900 z-50 flex flex-col">
                    <div className="p-6 border-b border-gray-800 flex justify-between items-center">
                        <button onClick={() => setViewingMapForJob(null)} className="h-10 w-10 rounded-full bg-gray-800 text-gray-400"><i className="fas fa-arrow-left"></i></button>
                        <h2 className="text-lg font-bold text-white">Client Map</h2>
                        <div className="w-10"></div>
                    </div>
                    <div className="flex-1 relative w-full bg-gray-800">
                        <MapContainer key={`${mapPosition[0]}`} center={mapPosition} zoom={15} style={{ height: '100%', width: '100%', zIndex: 1 }}><TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" /><Marker position={mapPosition}></Marker></MapContainer>
                        {/* NEW: Map Overlay Share Button */}
                        <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-[400] w-[90%]">
                            <button onClick={toggleLocationSharing} className={`w-full py-4 rounded-2xl font-bold shadow-lg flex items-center justify-center gap-2 ${isSharingLocation ? 'bg-red-500/90 text-white border border-red-400' : 'bg-teal-600 text-white'}`}>
                                <i className="fas fa-location-arrow"></i> {isSharingLocation ? 'Stop Sharing Location' : 'Start Navigating (Share Location)'}
                            </button>
                        </div>
                    </div>
                </div>
