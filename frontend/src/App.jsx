...
const AuthScreen = () => {
    ...
    return (
        <div className="bg-bgLight w-full max-w-md sm:max-w-lg md:max-w-xl mx-auto h-screen md:h-[850px] md:rounded-[2.5rem] md:shadow-2xl relative overflow-hidden md:border-8 md:border-gray-900 flex flex-col justify-center px-6">
...
const ClientApp = ({ socket, token }) => {
  ...
  return (
    <div className="bg-bgLight w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl mx-auto h-screen md:h-[850px] relative flex flex-col md:rounded-[2.5rem] md:shadow-2xl overflow-hidden text-gray-900 transition-all duration-300">
      <CallUI {...callLogic} />
          ...
          <div className="px-6 pt-6 pb-28 flex-1 flex flex-col w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {paginatedBookings.map(b => (
                    <div key={b._id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between">
...
const ProfessionalApp = ({ socket, token }) => {
    ...
    return (
        <div className="bg-gray-900 w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl mx-auto h-screen md:h-[850px] relative flex flex-col text-white md:rounded-[2.5rem] md:shadow-2xl overflow-hidden transition-all duration-300">
            <CallUI {...callLogic} />
                    ...
                    <div className="px-6 pt-10 pb-28 flex-1 flex flex-col w-full">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {paginatedJobs.map(job => (
                                <div key={job._id} className="bg-gray-800 p-5 rounded-2xl flex flex-col justify-between">
                                    ...
                                    <button onClick={() => handleViewMap(job)} className="bg-gray-700 text-teal-400 px-3 py-1 rounded text-xs mb-4 font-bold shadow-sm flex items-center self-start"><i className="fas fa-map-marker-alt mr-2"></i> View Map</button>
                                    <div className="flex gap-2 mt-auto"><button onClick={() => openChat(job)} className="flex-1 py-2 bg-gray-700 text-xs font-bold rounded-lg">Chat</button>{job.status === 'pending' && <button onClick={() => updateJobStatus(job._id, 'confirmed')} className="flex-1 py-2 bg-teal-600 text-xs font-bold rounded-lg">Accept</button>}{job.status === 'confirmed' && <button onClick={() => updateJobStatus(job._id, 'completed')} className="flex-1 py-2 bg-blue-600 text-xs font-bold rounded-lg">Complete</button>}</div>
                                </div>
                            ))}
                        </div>
