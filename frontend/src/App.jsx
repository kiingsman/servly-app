        ...
        {activeTab === 'chat' && activeChatRoom && (
          <div className="flex-1 flex flex-col w-full h-full bg-gray-50">
            <div className="bg-white px-6 py-4 flex justify-between items-center shadow-sm shrink-0 w-full">
                ...
                <div className="flex gap-3"><button onClick={callLogic.startCall} className="w-8 h-8 bg-teal-50 text-teal-600 rounded-full flex items-center justify-center text-xs hover:bg-teal-100 transition"><i className="fas fa-video"></i></button></div>
            </div>
            <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-4 w-full hide-scrollbar">
                {messageList.map((msg, i) => { const isMe = msg.senderId === user.id; return (<div key={i} className={`max-w-[75%] p-3 rounded-2xl text-sm shadow-sm ${isMe ? 'bg-teal-600 text-white self-end rounded-br-sm' : 'bg-white text-gray-800 self-start rounded-bl-sm border border-gray-100'}`}><p>{msg.message}</p><div className="flex items-center justify-end mt-1 gap-1"><span className={`text-[9px] ${isMe ? 'text-teal-100' : 'text-gray-400'}`}>{msg.time}</span>{isMe && <span className={`text-[10px] ${msg.isRead ? 'text-blue-300' : 'text-teal-200'}`}>{msg.isRead ? '✓✓' : '✓'}</span>}</div></div>); })}
                <div ref={chatEndRef} />
            </div>
        ...
        <div className="absolute inset-0 w-full h-full bg-gray-50 z-[70] flex flex-col hide-scrollbar animate-[fadeIn_0.3s_ease-out]">
            ...
            <div className="w-full flex-1 p-6 overflow-y-auto hide-scrollbar">
                ...
                <form onSubmit={handleBookingSubmit} className="w-full">
                    ...
                    <button type="submit" className="w-full bg-teal-600 text-white font-bold py-4 rounded-2xl shadow-lg shadow-teal-500/30 hover:bg-teal-700 transition">Confirm Booking</button>
                </form>
            </div>
        </div>
      )}
      ...
      {viewingLiveMap && (
        <div className="absolute inset-0 w-full h-full bg-white z-[90] flex flex-col animate-[slideUp_0.3s_ease-out]">
            <div className="px-6 py-4 flex justify-between items-center bg-white shadow-sm shrink-0 w-full">
                ...
                <button onClick={() => setViewingLiveMap(false)} className="text-gray-500 hover:text-gray-800"><i className="fas fa-times"></i></button>
            </div>
            <div className="flex-1 w-full relative min-h-0">
                ...
                <button onClick={toggleLocationSharing} className={`absolute bottom-6 flex items-center gap-2 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-full text-white font-bold text-sm shadow-xl z-[400] transition ${isSharingLocation ? 'bg-red-500 hover:bg-red-600' : 'bg-teal-600 hover:bg-teal-700'}`}><i className={`fas ${isSharingLocation ? 'fa-stop-circle' : 'fa-location-arrow'}`}></i>{isSharingLocation ? 'Stop Sharing' : 'Share My Location'}</button>
            </div>
        </div>
      )}
...
const ProfessionalApp = ({ socket, token }) => {
    ...
    return (
        <div className="w-full h-screen relative flex flex-col bg-gray-900 text-white overflow-hidden font-sans">
            ...
            {/* INDEPENDENT SCROLLING TAB WRAPPER */}
            <div className="flex-1 w-full h-full flex flex-col overflow-hidden relative">
                ...
                {activeTab === 'chat' && activeChatRoom && (
                  <div className="flex-1 flex flex-col w-full h-full bg-gray-900">
                    <div className="bg-gray-800 px-6 py-4 flex justify-between items-center shadow-sm shrink-0 w-full">
                        <div className="flex items-center"><button onClick={() => setActiveTab('jobs')} className="mr-4 text-gray-400 hover:text-white transition"><i className="fas fa-arrow-left"></i></button><div><h3 className="font-bold text-white text-sm">{activeChatRoom.clientName}</h3><p className="text-[10px] text-teal-400 font-bold">Job Chat</p></div></div>
                        <div className="flex gap-3"><button onClick={callLogic.startCall} className="w-8 h-8 bg-gray-700 text-teal-400 rounded-full flex items-center justify-center text-xs hover:bg-gray-600 transition"><i className="fas fa-video"></i></button></div>
                    </div>
                    <div className="flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-4 w-full hide-scrollbar">
            ...
            {/* ROOT MAP OVERLAY FOR PROFESSIONAL */}
            {viewingMapForJob && (
