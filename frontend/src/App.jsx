...
const ClientApp = ({ socket, token }) => {
  ...
  return (
    <div className="bg-bgLight w-full h-screen relative flex flex-col overflow-hidden text-gray-900">
      ...
      {/* FULL WIDTH HEADER */}
      <div className="bg-white px-6 py-4 shadow-sm flex justify-between items-center z-10 sticky top-0 w-full">
      ...
      {/* MAIN TABS CONTAINER: Fully stretched across the screen */}
      <div className="w-full flex-1 overflow-y-auto hide-scrollbar flex flex-col relative">
        
        {activeTab === 'home' && (
          <div className="w-full px-6 pt-6 pb-28 flex flex-col">
        ...
        {activeTab === 'chat' && activeChatRoom && (
          <div className="w-full flex-1 flex flex-col bg-gray-50 relative pb-20">
            <div className="bg-white px-6 py-4 flex items-center justify-between shadow-sm sticky top-0 z-20 w-full"><div className="flex items-center"><button onClick={() => setActiveTab('bookings')} className="mr-4 text-gray-400"><i className="fas fa-arrow-left"></i></button><div><h3 className="font-bold text-gray-800 text-sm">{activeChatRoom.professionalName}</h3><p className="text-[10px] text-teal-600 font-bold">Booking Chat</p></div></div><div className="flex gap-3"><button onClick={callLogic.startCall} className="w-8 h-8 bg-teal-50 text-teal-600 rounded-full flex items-center justify-center text-xs"><i className="fas fa-video"></i></button></div></div>
            <div className="w-full flex-1 overflow-y-auto px-6 py-6 flex flex-col gap-4">{messageList.map((msg, i) => { const isMe = msg.senderId === user.id; return (<div key={i} className={`max-w-[75%] p-3 rounded-2xl text-sm shadow-sm ${isMe ? 'bg-primary text-white self-end rounded-br-sm' : 'bg-white text-gray-800 self-start rounded-bl-sm border border-gray-100'}`}><p>{msg.message}</p><div className="flex items-center justify-end mt-1 gap-1"><span className={`text-[9px] ${isMe ? 'text-teal-100' : 'text-gray-400'}`}>{msg.time}</span>{isMe && <span className={`text-[10px] ${msg.isRead ? 'text-blue-300' : 'text-teal-200'}`}>{msg.isRead ? '✓✓' : '✓'}</span>}</div></div>); })}<div ref={chatEndRef} /></div>
            
            <div className="absolute bottom-0 w-full bg-white py-4 px-6 border-t border-gray-100 flex items-center gap-2">
                ...
                <input type="text" placeholder="Type message..." value={currentMessage} onChange={(e) => setCurrentMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && sendMessage()} className="flex-1 w-full bg-gray-50 py-3 px-4 rounded-full text-sm outline-none border border-gray-100" />
                <button onClick={sendMessage} className="flex-shrink-0 w-10 h-10 bg-primary text-white rounded-full flex items-center justify-center shadow-md"><i className="fas fa-paper-plane"></i></button>
            </div>

            {viewingLiveMap && (
              <div className="absolute inset-0 w-full h-full bg-white z-30 flex flex-col">
                  <div className="px-6 py-4 flex justify-between items-center bg-white shadow-sm z-40 w-full">
        ...
        {activeTab === 'chat' && !activeChatRoom && (
            <div className="w-full flex-1 flex flex-col items-center justify-center text-gray-400 pb-28 px-6">
      ...
      {viewingProfile && !bookingPro && (
        <div className="absolute inset-0 w-full h-screen bg-white z-50 overflow-y-auto flex flex-col animate-[slideUp_0.3s_ease-out]">
            <div className="w-full relative h-64 bg-gray-100 px-6">
            ...
            <div className="w-full px-6 -mt-16 relative z-10 pb-10">
                <div className="w-full bg-white rounded-3xl p-6 shadow-xl border border-gray-50">
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <div className="bg-teal-50 text-primary px-3 py-1.5 rounded-xl font-black text-sm flex-shrink-0">₦{viewingProfile.price}<span className="text-[10px] text-teal-600/60 ml-1">/hr</span></div>
                    </div>
      ...
      {bookingPro && !isBookingSuccess && (
        <div className="absolute inset-0 w-full h-screen bg-bgLight z-[60] flex flex-col">
            ...
            <div className="w-full flex-1 p-6 overflow-y-auto hide-scrollbar">
                ...
                <form onSubmit={handleBookingSubmit} className="w-full">
                    ...
                    <div className="mb-8 w-full"><label className="text-xs font-bold text-gray-500 ml-1">Service Address</label><textarea required placeholder="Enter full address..." value={bookingData.address} onChange={e => setBookingData({...bookingData, address: e.target.value})} className="w-full bg-white border border-gray-200 p-4 rounded-xl mt-1 outline-none text-sm font-medium shadow-sm h-24 resize-none" /></div>
      ...
      {isBookingSuccess && (
        <div className="absolute inset-0 w-full h-screen bg-primary z-[70] flex flex-col items-center justify-center px-6 py-8 text-center animate-[fadeIn_0.3s_ease-out]">
            ...
            <button onClick={() => { setIsBookingSuccess(false); setBookingPro(null); setViewingProfile(null); setActiveTab('bookings'); }} className="w-full bg-white text-primary font-black py-4 px-12 rounded-2xl shadow-xl hover:bg-gray-50 transition">View My Bookings</button>
        </div>
      )}

      {/* FULL WIDTH BOTTOM NAV BAR */}
      <div className="absolute bottom-0 w-full bg-white border-t border-gray-100 shadow-[0_-10px_40px_rgba(0,0,0,0.03)] z-10 flex justify-around py-4 px-6 pb-6">
...
const ProfessionalApp = ({ socket, token }) => {
    ...
    return (
        <div className="w-full h-screen relative flex flex-col bg-gray-900 text-white overflow-hidden">
            <CallUI {...callLogic} />
            
            {/* MAIN TABS CONTAINER: Fully stretched */}
            <div className="w-full flex-1 overflow-y-auto hide-scrollbar flex flex-col relative">
                
                {activeTab === 'jobs' && (
                    <div className="w-full px-6 pt-10 pb-28 flex flex-col">
                        ...
                        {paginatedJobs.map(job => (
                            <div key={job._id} className="w-full bg-gray-800 p-5 rounded-2xl mb-4 hover:bg-gray-750 transition shadow-sm">
                                ...
                                <div className="flex gap-2 w-full"><button onClick={() => openChat(job)} className="flex-1 py-2 bg-gray-700 text-xs font-bold rounded-lg hover:bg-gray-600 transition">Chat</button>{job.status === 'pending' && <button onClick={() => updateJobStatus(job._id, 'confirmed')} className="flex-1 py-2 bg-teal-600 text-xs font-bold rounded-lg hover:bg-teal-500 transition">Accept</button>}{job.status === 'confirmed' && <button onClick={() => updateJobStatus(job._id, 'completed')} className="flex-1 py-2 bg-blue-600 text-xs font-bold rounded-lg hover:bg-blue-500 transition">Complete</button>}</div>
                            </div>
                        ))}
                ...
                {activeTab === 'chat' && activeChatRoom && (
                  <div className="w-full flex-1 flex flex-col bg-gray-900 z-20 pb-20 relative">
                    ...
                    <div className="absolute bottom-0 w-full bg-gray-800 py-4 px-6 border-t border-gray-700 flex items-center gap-2"><div className="relative flex-shrink-0"><button onClick={toggleLocationSharing} className={`w-10 h-10 rounded-full flex items-center justify-center transition hover:bg-gray-600 ${isSharingLocation ? 'bg-red-500/20 text-red-500' : 'bg-gray-700 text-gray-400'}`}><i className="fas fa-map-marker-alt"></i></button>{isSharingLocation && <div className="absolute top-0 right-0 w-3 h-3 bg-red-500 rounded-full animate-ping"></div>}</div><input type="text" placeholder="Type message..." value={currentMessage} onChange={(e) => setCurrentMessage(e.target.value)} onKeyPress={e => e.key === 'Enter' && sendMessage()} className="w-full flex-1 bg-gray-700 text-white py-3 px-4 rounded-full text-sm outline-none placeholder-gray-400 focus:ring-1 focus:ring-teal-500" /><button onClick={sendMessage} className="flex-shrink-0 w-10 h-10 bg-teal-600 text-white rounded-full flex items-center justify-center shadow-md hover:bg-teal-500 transition"><i className="fas fa-paper-plane"></i></button></div>
                  </div>
                )}

                {activeTab === 'chat' && !activeChatRoom && (
                    <div className="w-full flex-1 flex flex-col items-center justify-center text-gray-500 pb-28 px-6">
            ...
            {viewingMapForJob && (
              <div className="absolute inset-0 w-full h-screen bg-gray-900 z-50 flex flex-col animate-[slideUp_0.3s_ease-out]">
                  <div className="w-full bg-gray-800 px-6 py-4 flex items-center shadow-sm z-10 relative">
            ...
            {/* FULL WIDTH BOTTOM NAV BAR */}
            <div className="absolute bottom-0 w-full bg-gray-800 border-t border-gray-700 flex justify-around py-4 px-6 pb-6 z-10">
